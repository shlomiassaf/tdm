...
export class JSONAPIDeserializeMapper extends DeserializeMapper {
  ...
  protected getIncluded(
    rel: ResourceIdentifierObject,
    prop?: PropMetadata
  ): any {
    ...
    const included =
      japiUtils.findIncluded(this.source.included, rel) || (rel as any);

    if (included) {
      const target: any =
        prop && targetStore.hasTarget(prop.type)
          ? prop.type
...
export class JSONAPISerializeMapper extends SerializeMapper {
  ...
  serialize(container: PropertyContainer): TopLevel {
    ...
    if (Array.isArray(this.source)) {
      ...
      this.doc.data = this.serializeCollection(
        this.source,
        this.doc,
        container
      );
    ...
    if (!(this instanceof JSONAPIChildSerializeMapper) && this.cache.size > 0) {
      ...
      Array.from(this.cache.values()).forEach(toInclude => {
    ...
    if (obj) {
      if (obj.attributes) {
        delete obj.attributes[
          targetStore.getIdentityKey(container.target, 'outgoing')
        ];
    ...
    const cb = (pMap: PoClassPropertyMap) => {
      ...
      if (p && p.relation) {
        ...
        if (!type || !name) {
          ...
          throw new Error(
            'Property Relation without type or name, try setting typeGetter in prop'
          );
        }

        const isArray =
          p.relation && p.type.isArray && Array.isArray(obj[pMap.cls]);
        ...
        if (isArray) {
          ...
          data.relationships[pMap.cls] = {
            data: createRel(obj[pMap.cls][idKey])
          };
  ...
  private serializeChild(obj: any, type: any): void {
    targetStore.serialize(
      type,
      new JSONAPIChildSerializeMapper(obj, this.cache, this.plainMapper)
    );
  }
  ...
  private serializeCollection(
    ...
    doc: TopLevel,
    container: PropertyContainer
  ): Array<ResourceObject | ResourceIdentifierObject> {
