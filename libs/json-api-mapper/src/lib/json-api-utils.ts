...
export function findIncluded(
  included: J.ResourceObject[],
  identifierObject: J.ResourceIdentifierObject
): J.ResourceObject | undefined {
  if (Array.isArray(included)) {
    return included.find(
      i => i.type === identifierObject.type && i.id == identifierObject.id
    );
  }
}

export function getType(
  obj: J.TopLevel | J.ResourceIdentifierObject | J.RelationshipObject
): string | undefined {
...
export function getResourceKeys(
  resource: J.ResourceObject
): { att: string[]; rel: string[] } {
  return {
    att:
      resource && resource.attributes ? Object.keys(resource.attributes) : [],
    rel:
...
export function getDocumentKeys(
  ...
  return Array.isArray(doc.data)
    ? getResourceKeys(doc.data[0])
    : getResourceKeys(doc.data);
}
