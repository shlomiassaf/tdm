  ...
  describe('Active Record State', () => {
    ...
    it('should not emit error, only error event', () => {
      return eventConsumer(bucket.create(User))
        ...
        .then(events =>
          expect((<any>events[1]).error.toString()).toBe('Error: testError')
        );
    });
      ...
      return bucket
        ...
        .catch(err =>
          expect(err.message).toEqual(
            'Model Error [User]: Call to next() while not in an active action.'
          )
        );
    });
    ...
    it('should reject the next action on error', () => {
      return bucket
        .create(User)
        .$get({ throwError: new Error('testError') })
        .next()
            ...
            .onEvent(event => {
              switch (event.type) {
                ...
                case 'ActionCancel':
                  ...
                  break;
                case 'ActionEnd':
                  expect((event as ActionEndResourceEvent).result).toBe(
                    'cancel'
                  );
                  expect(user.$rc.busy).toBe(false);
                  break;
        ...
        .then(() => {
          return eventConsumer(user)
            ...
            .onEvent(event => {
              switch (event.type) {
                case 'ActionError':
                  expect((<any>event).error.toString()).toEqual(
                    'Error: testError'
                  );
                  break;
              }
