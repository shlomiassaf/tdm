...
describe('NG-HTTP', () => {
  describe('HTTP Resource Action Handler', () => {
      ...
      return eventConsumer(new User())
        ...
        .onEvent(event => {
          if (event.type === 'ActionError') {
            expect(event['error'].toString()).toEqual(
              'Error: HttpClientResourceModule did not init, are you trying to invoke an action before the modules registered?'
            );
          }
        })
        .run(ec => ec.ar.$get());
    ...
    describe('URL Parsing', () => {
      ...
      it(
        'should leave as is trailing slashes',
        fakeAsync(() => {
          @HttpResource({
            endpoint: '/api/users/'
          })
      ...
      it(
        'should build resource with default param ',
        fakeAsync(() => {
          class UserBase {
            @Identity()
            @UrlParam()
      ...
      it(
        'should replace http on each call',
        fakeAsync(() => {
          @HttpResource({
            endpoint: '/api/users'
          })
      ...
      it(
        'should throw if id not supplied and not optional',
        fakeAsync(() => {
          @HttpResource({
            endpoint: '/api/users/:id'
          })
          ...
          expect(error.toString()).toEqual(
            'Error: URL Parameter Error in HttpAdapter: Expected "id" to be defined'
          );
        })
      );

      it(
        'should build resource with bound param',
        fakeAsync(() => {
          class User_ {
            @Identity()
            @UrlParam()
      ...
      it(
        'should use urlTemplateParamName, if defined',
        fakeAsync(() => {
          class User_ {
            @Identity()
            @UrlParam()
      ...
      it(
        'should apply method filter, if set.',
        fakeAsync(() => {
          class User_ {
            @Identity()
            @UrlParam()
      ...
      it(
        'should set non path param as query string',
        fakeAsync(() => {
          class User_ {
            @UrlParam() qs1 = 1;
          }
      ...
      it(
        ...
        fakeAsync(() => {
          ...
          const req = httpMock.expectOne(
            '/api/users?qs1=param1-instance&qs2=param2-adhoc'
          );
          req.flush([]);
        })
      );
