...
import { FormModel, FormProp, ngFormsMapper, clone, createControl } from '@tdm/ngx-dynamic-forms';

function serialize(instance: any): FormGroup {
  return targetStore.serialize(instance.constructor, ngFormsMapper.serializer(instance));
}
...
describe('@tdm/ngx-dynamic-forms', () => {
  describe('angular-forms-mapper', () => {
    ...
    it('should add custom async validation', fakeAsync(() => {
        ...
        @FormProp(
          {
            asyncValidators: [
              ...
              (value: AbstractControl) => Promise.resolve(null)
                .then(() => ({ alwaysFail: 'alwaysFail failed' })
            ]
          }
        )
        required: string;
    ...
    describe('Arrays', () => {
      ...
      it('should serialize and deserialize array of child forms', () => {
        ...
        user.id = 1;
        user.addresses = [
          Object.assign(new Address(), { street: '1 Test St', city: 'testomania1', zip: 123 }),
          Object.assign(new Address(), { street: '2 Test St', city: 'testomania2', zip: 456 })
        ];
        ...
        expect((controls[0] as FormGroup).getRawValue()).toEqual(user.addresses[0]);
        expect(controls[1]).toBeInstanceOf(FormGroup);
        expect((controls[1] as FormGroup).getRawValue()).toEqual(user.addresses[1]);


        const thirdAddress = { street: '3 Test St', city: 'testomania3', zip: 456 };
        (formGroup.get('addresses') as  FormArray).removeAt(1);
        (formGroup.get('addresses') as  FormArray).push(serialize(Object.assign(new Address(), thirdAddress)));
    ...
    describe('flattening', () => {
      ...
      it('should serialize and deserialize array of flatten definition', () => {
        ...
        const fourthAddress = { street: '4 Test St', city: 'testomania4', zip: 789 };
        const fourthAddressFormGroup = clone(controls[0] as FormGroup, fourthAddress);
    ...
    describe('createControl', () => {
      ...
      it('should create and populate a form control from a property with flatten expression of Array', () => {
        const formGroup = serialize(user);
        const manual = new FormArray(user.addresses.map( addr => createControl(User, 'addresses', addr) ) );
        expect(manual.getRawValue()).toEqual( (formGroup.get(`addresses`) as FormArray).getRawValue());
      });
      ...
      it('should create and populate a form control from a nested property using a deep path ', () => {
        const formGroup = serialize(user);
        const manual = createControl(User, ['addresses', 'additional.other'], createAddress(10));
      ...
      it('should create a form control from a nested property starting with a childForm and then a flatten expression', () => {
        const manual = createControl(User, ['childModel', 'childModel.addressCollection.additional.other']);
        expect(manual).toBeInstanceOf(FormGroup);
        expect(manual.get('street')).toBeInstanceOf(FormControl);
        expect(manual.get('city')).toBeInstanceOf(FormControl);
        expect(manual.get('zip')).toBeInstanceOf(FormControl);
      ...
      it('should create and populate a form control from a nested property starting with a childForm and then a flatten expression', () => {
        const formGroup = serialize(user);
        const manual = createControl(User, ['childModel', 'childModel.addressCollection.additional.other'], createAddress(15));
        expect(manual.getRawValue()).toEqual( (formGroup.get(`childModel.childModel.addressCollection.0.additional.other.2`) as FormArray).getRawValue());
      });
    });
  });
});
