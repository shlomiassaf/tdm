...
new User().$get().parent.$rc.next().then( value => {
 ...
 * @tssert
 * @tsError 2322
 * @tsErrorMsg Type 'TDMModel<TDMModel<ActiveRecord<User_>> & User_ & TDMModel<User_> & HttpActiveRecord> & TDMModel<A...' is not assignable to type 'string'.
 * @loc 2:9
 */
new User().$get().parent.$rc.next().then( value => {
...
class Y extends X<User> {
  constructor(user: User) {
    ...
    this.user.$get().parent.$rc.self$.subscribe( value => {
      this.user = value;
    });
  }
}
